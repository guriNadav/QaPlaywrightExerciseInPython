...
class SearchResultsPage(BasePage):
    ...
    def apply_max_price(self, max_price: float) -> None:
        ...
        if max_input.is_visible():
            ...
            # המתנה עד שהתוצאות החדשות יטענו ויהיו נראות
    ...
    def go_to_next_page(self) -> bool:
        ...
        if next_elem.is_visible():
            next_elem.click()
            # המתנה לסיום טעינת הרשת לאחר המעבר לעמוד הבא
...
#     Provides helpers to filter by maximum price, retrieve item locators, and
...
#     async def apply_max_price(self, max_price: float) -> None:
#         """Set the maximum price filter if the input is visible.
...
#             await self.page.wait_for_selector(self.items_selector, state="visible")
